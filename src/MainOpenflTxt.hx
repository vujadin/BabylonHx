...
class MainOpenflTxt extends Application {
	...
	public override function create (config:Config):Void {
		...
		super.create (config);
		_stage = new Stage (this.window.width, this.window.height, 0xFFFFFFFF);
	...
	public function createSquares(){
		...
		fill.graphics.drawRect (0, 0, 200, 200);
		fill.x = -50;
		fill.y = -50;
		...
		creatTxt(fpsContainer);

	}

	public function creatTxt(fpsContainer:Sprite){
		fpsContainer.addChild(new openfl.display.FPS(5, 5, 0xFFFFFF));
		_stage.addChild (fpsContainer);
	}
