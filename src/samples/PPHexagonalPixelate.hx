...
class PPHexagonalPixelate {
	...
	public function new(scene:Scene) {
		...
		var hexagonalPixelatePP = new HexagonalPixelatePostProcess("hexagonalPixelate_PP", 1.0, camera);
		...
		scene.getEngine().runRenderLoop(function () {
			scene.render();
			#if openfl
			@:privateAccess scene.getEngine().gl.enable(scene.getEngine().gl.BLEND);
			#end
		});
	}
