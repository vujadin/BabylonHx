...
#define volsteps 3
#define stepsize 0.490
 
#define zoom 1.900
#define tile   0.850
#define speed2  0.10
 
#define brightness 0.046
...
void main() {   
	...
	float time2 = time;               
	float speed = speed2;
	speed = -.02 * cos(time2*0.002 + 3.1415926/200.0);          
	//speed = 0.0;	
...
*/
@:expose('BABYLON.Starfield2ProceduralTexture') class Starfield2 extends ProceduralTexture {
	...
	public function new(name:String, size:Float, scene:Scene, ?fallbackTexture:Texture, ?generateMipMaps:Bool) {
		if (!ShadersStore.Shaders.exists("starfield2textureFragmentShader")) {
			ShadersStore.Shaders.set("starfield2textureFragmentShader", fragmentShader);
		}
		
		super(name, size, "starfield2texture", scene, fallbackTexture, generateMipMaps);
