...
class TriPlanarMaterial extends Material {
	...
	private function _checkCache(scene:Scene, ?mesh:AbstractMesh, useInstances:Bool = false):Bool {
		...
		if (this._defines.defines[TPMD.INSTANCES] != useInstances) {
			return false;
		}
	...
	override public function isReady(?mesh:AbstractMesh, useInstances:Bool = false):Bool {
		...
		if (scene.clipPlane != null) {
			this._defines.defines[TPMD.CLIPPLANE] = true;
		}
		
		if (engine.getAlphaTesting()) {
			this._defines.defines[TPMD.ALPHATEST] = true;
		}
		
		// Point size
		if (this.pointsCloud || scene.forcePointsCloud) {
			this._defines.defines[TPMD.POINTSIZE] = true;
		}
		
		// Fog
		if (scene.fogEnabled && mesh != null && mesh.applyFog && scene.fogMode != Scene.FOGMODE_NONE && this.fogEnabled) {
			this._defines.defines[TPMD.FOG] = true;
		}
		...
		if (scene.lightsEnabled && !this.disableLighting) {
			for (index in 0...scene.lights.length) {
				...
				needNormals = true;
				this._defines.defines[TPMD.LIGHT0 + lightIndex] = true;
				...
				if (!light.specular.equalsFloats(0, 0, 0)) {
					this._defines.defines[TPMD.SPECULARTERM] = true;
				}
				...
				if (scene.shadowsEnabled) {
					...
					if (mesh != null && mesh.receiveShadows && shadowGenerator != null) {
						this._defines.defines[TPMD.SHADOW0 + lightIndex] = true;
						
						this._defines.defines[TPMD.SHADOWS] = true;
						
						if (shadowGenerator.useVarianceShadowMap || shadowGenerator.useBlurVarianceShadowMap) {
							this._defines.defines[TPMD.SHADOWVSM0 + lightIndex] = true;
						}
						
						if (shadowGenerator.usePoissonSampling) {
							this._defines.defines[TPMD.SHADOWPCF0 + lightIndex] = true;
						}
					}
				}
		...
		if (mesh != null) {
			if (needNormals && mesh.isVerticesDataPresent(VertexBuffer.NormalKind)) {
				this._defines.defines[TPMD.NORMAL] = true;
			}
			if (mesh.useVertexColors && mesh.isVerticesDataPresent(VertexBuffer.ColorKind)) {
				this._defines.defines[TPMD.VERTEXCOLOR] = true;
				
				if (mesh.hasVertexAlpha) {
					this._defines.defines[TPMD.VERTEXALPHA] = true;
				}
			}
			if (mesh.useBones && mesh.computeBonesUsingShaders) {
				this._defines.defines[TPMD.BONES] = true;
				this._defines.BonesPerMesh = (mesh.skeleton.bones.length + 1);
				this._defines.defines[TPMD.BONES4] = true;
			}
			
			// Instances
			if (useInstances) {
				this._defines.defines[TPMD.INSTANCES] = true;
			}
		}
		...
		if (!this._defines.isEqual(this._cachedDefines)) {
			...
			if (this._defines.defines[TPMD.FOG]) {
				fallbacks.addFallback(1, "FOG");
			}
			
			for (lightIndex in 0...Material.maxSimultaneousLights) {
				if (!this._defines.defines[TPMD.LIGHT0 + lightIndex]) {
					continue;
				}
				...
				if (this._defines.defines[TPMD.SHADOW0 + lightIndex]) {
					fallbacks.addFallback(0, "SHADOW" + lightIndex);
				}
				
				if (this._defines.defines[TPMD.SHADOWPCF0 + lightIndex]) {
					fallbacks.addFallback(0, "SHADOWPCF" + lightIndex);
				}
				
				if (this._defines.defines[TPMD.SHADOWVSM0 + lightIndex]) {
					fallbacks.addFallback(0, "SHADOWVSM" + lightIndex);
				}
			}
		 
			if (this._defines.defines[TPMD.BONES4]) {
			...
			if (this._defines.defines[TPMD.NORMAL]) {
				attribs.push(VertexBuffer.NormalKind);
			}
			
			if (this._defines.defines[TPMD.VERTEXCOLOR]) {
				attribs.push(VertexBuffer.ColorKind);
			}
			
			if (this._defines.defines[TPMD.BONES]) {
				attribs.push(VertexBuffer.MatricesIndicesKind);
				attribs.push(VertexBuffer.MatricesWeightsKind);
			}
			
			if (this._defines.defines[TPMD.INSTANCES]) {
				attribs.push("world0");
				attribs.push("world1");
				attribs.push("world2");
				attribs.push("world3");
	...
	override public function bind(world:Matrix, ?mesh:Mesh) {
		...
		if (this._defines.defines[TPMD.SPECULARTERM]) {
	...
	override public function clone(name:String, cloneChildren:Bool = false):TriPlanarMaterial {
	...
	override public function serialize():Dynamic {	
		...
		serializationObject.customType      = "TerrainMaterial";
	...
	public static function Parse(source:Dynamic, scene:Scene, rootUrl:String):TriPlanarMaterial {
		...
		if (source.diffuseTextureX != null) {
			material.diffuseTextureX = cast Texture.Parse(source.diffuseTextureX, scene, rootUrl);
		}
		
		if (source.diffuseTextureY != null) {
			material.diffuseTextureY = cast Texture.Parse(source.diffuseTextureY, scene, rootUrl);
		}
		
		if (source.diffuseTextureZ != null) {
			material.diffuseTextureZ = cast Texture.Parse(source.diffuseTextureZ, scene, rootUrl);
		}
