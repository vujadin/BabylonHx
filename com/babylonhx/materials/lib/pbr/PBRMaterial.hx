...
class PBRMaterial extends Material {
	...
	private static function _convertColorToLinearSpaceToRef(color:Color3, ref:Color3, useScalarInLinear:Bool = false) {
	...
	public static function BindLights(scene:Scene, mesh:AbstractMesh, effect:Effect, defines:PBRMaterialDefines, useScalarInLinearSpace:Bool = false) {
		...
		for (index in 0...scene.lights.length) {
			...
			_lightRadiuses[lightIndex] = light.radius;
		...
		effect.setFloat4("vLightRadiuses", _lightRadiuses[0],
                _lightRadiuses[1],
                _lightRadiuses[2],
                _lightRadiuses[3]);
	}
	...
	override public function bind(world:Matrix, ?mesh:Mesh) {
		...
		if (this._myScene.getCachedMaterial() != this) {
			...
			if (this._myScene.texturesEnabled) {
				...
				if ((this.reflectionTexture != null || this.refractionTexture != null) && this._myScene.getEngine().getCaps().textureLOD) {
