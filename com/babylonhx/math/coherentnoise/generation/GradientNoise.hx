...
/// </summary>
class GradientNoise extends Generator {
	...
	private var m_SCurve:Float->Float;
	...
	public function new(seed:Int, ?sCurve:Float->Float) {
		super();
	...
	public var S_Curve(get, never):Float->Float;
	private inline function get_S_Curve():Float->Float { 
		return m_SCurve != null ? m_SCurve : SCurve.DefaultInterpolate;
	}
	...
	override public function GetValue(x:Float, y:Float, z:Float):Float {
		...
		var n0 = GetNoise(x, y, z, ix, iy, iz);
		var n1 = GetNoise(x, y, z, ix + 1, iy, iz);
	...
	function GetRandomVector(x:Int, y:Int, z:Int):Vector3 {
		if (Period > 0) {
		...
		return new Vector3(Vectors[vectorIndex], Vectors[vectorIndex + 1], Vectors[vectorIndex + 2]);
	}

	private function GetNoise(x:Float, y:Float, z:Float, ix:Int, iy:Int, iz:Int):Float {
		var gradient = GetRandomVector(ix, iy, iz);
		return Vector3.Dot(gradient, new Vector3(x - ix, y - iy, z - iz)) * 2.12; // scale to [-1,1]
	}
