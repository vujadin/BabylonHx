...
class GLTFLoader implements IGLTFLoader {
	...
	public var _gltf:IGLTF;
	public var _babylonScene:Scene;

	private var _disposed:Bool = false;
	private var _parent:GLTFFileLoader;
	private var _rootUrl:String;
	private var _defaultMaterial:PBRMaterial;
	private var _defaultSampler = {} as IGLTFSampler;
	private var _rootNode:IGLTFNode;
	private var _successCallback:Void-Void;
	private var _renderReady:Bool = false;

	private var _renderReadyObservable:Observable<GLTFLoader> = new Observable<GLTFLoader>();

	// Count of pending work that needs to complete before the asset is rendered.
	private var _renderPendingCount:Int = 0;

	// Count of pending work that needs to complete before the loader is disposed.
	private var _loaderPendingCount:Int = 0;

	private var _loaderTrackers:Array<GLTFLoaderTracker> = [];

	public static var Extensions:Map<String, GLTFLoaderExtension> = new Map();

	public static function RegisterExtension(extension:GLTFLoaderExtension) {
		if (GLTFLoader.Extensions[extension.name] != null) {
	...
	public function new(parent:GLTFFileLoader) {
		this._parent = parent;
	}
	...
	public function importMeshAsync(meshesNames:Dynamic, scene:Scene, data:IGLTFLoaderData, rootUrl:String, onSuccess:Array<AbstractMesh>->Array<particleSystems>->Array<ParticleSystem>->Array<Skeleton>->Void, onProgress:Dynamic, onError:Dynamic) {
		this._loadAsync(meshesNames, scene, data, rootUrl, function() {
			onSuccess(this._getMeshes(), [], this._getSkeletons());
		}, onProgress, onError);
	}

	public function loadAsync(scene:Scene, data:IGLTFLoaderData, rootUrl:String, onSuccess:Void->Void onProgress:Dynamic, onError:Dynamic) {
		this._loadAsync(null, scene, data, rootUrl, onSuccess, onProgress, onError);
	}

	private function _loadAsync(nodeNames:Dynamic, scene:Scene, data:IGLTFLoaderData, rootUrl:String, onSuccess:Void->Void, onProgress:Dynamic, onError:Dynamic) {
	...
	public function _executeWhenRenderReady(func:Void->Void) {
	...
	private function _onRenderReady() {
	...
	private function _onComplete() {
		if (this._parent.onComplete != null) {
	...
	private function _loadData(data:IGLTFLoaderData) {
		this._gltf = cast data.json;
		...
		if (data.bin != null) {
			...
			if (buffers != null && buffers[0] != null && buffers[0].uri == null) {
	...
	private function _getMeshes():Array<AbstractMesh> {
	...
	private function _getSkeletons():Array<Skeleton> {
